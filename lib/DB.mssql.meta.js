...
DBmeta.prototype.getTableFields = function(tabledef, callback){
  ...
  _this.db.Recordset('',"select \
      ...
      inner join sys.schemas on sys.schemas.schema_id = sys.tables.schema_id \
      inner join sys.types on sys.columns.system_type_id = sys.types.user_type_id \
    ...
    for(var i=0;i<rslt.length;i++){
      ...
      if(col.type_name=="varchar" || 
        ...
        if(field.length==-1){
          messages.push('WARNING - Column '+tabledef.schema+'.'+tabledef.name+'.'+col.column_name+': UNLIMITED ' + col.type_name + ' COLUMN LENGTH NOT SUPPORTED, USING 99999');
          field.length = 99999;
        }
      }
      ...
      else if(col.type_name=="money"){ field.type = "money"; field.precision = [34, 4]; }

      else if(col.type_name=="varbinary"){ field.type = "varbinary"; field.length = col.max_length; }
      else{
        messages.push('WARNING - Skipping Table: '+tabledef.schema+'.'+tabledef.name+'.'+col.column_name+': Data type '+col.type_name + ' not supported.');
        continue;
      }
      field.coldef = col;
      fields.push(field);
