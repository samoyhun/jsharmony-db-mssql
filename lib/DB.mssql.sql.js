...
exports.getSearchTerm = function (ent, field, pname, search_value, comparison) {
  ...
  var dbtype = null;
  var pname_param = XtoDB(ent, field, '@' + pname);
  switch (ftype) {
    case 'bit':
      dbtype = types.Bit;
...
function XfromDB(ent, field, fieldsql){
  ...
  if(field.type && field.sql_from_db){
    var rparams = XgetParams(field, fieldsql);
    var rslt = field.sql_from_db;
    for(var rparam in rparams) rslt = DB.util.ReplaceAll(rslt, '%%%'+rparam+'%%%', rparams[rparam]);
  }
  //Simplify
  if(rslt == field.name) {}
...
function XtoDB(ent, field, fieldsql){
  ...
  if(field.type && field.sql_to_db){
    var rparams = XgetParams(field, fieldsql);
    var rslt = field.sql_to_db;
    for(var rparam in rparams) rslt = DB.util.ReplaceAll(rslt, '%%%'+rparam+'%%%', rparams[rparam]);
  }
  return rslt;
}

function XgetParams(field, fieldsql){
  var rparams = field.sql_conversion_defaults||{};
  rparams.FIELD = field.name;
  rparams.SQL = fieldsql;
