...
exports.getModelRecordset = function (ent, model, sql_filterfields, allfields, sortfields, searchfields, datalockqueries,
  ...
  if (('sqlwhere' in model) && model.sqlwhere) sql_suffix += parseSQL(ent, model.sqlwhere);
  else sql_suffix += '1=1';
  ...
  if (!('sqlselect' in model)) {
    ...
    if(global.dbconfig && (global.dbconfig.version == "mssql2008")){
      var sql2 = '';
...
exports.getModelMultisel = function (ent, model, lovfield, allfieldslist, sql_filterkeys, datalockqueries, lov_datalockqueries, param_datalocks) {
  ...
  if (!('sqlselect' in model)) {
    ...
    sql += ') ' + tbl;
    sql += ' full outer join (%%%LOVSQL%%%) multiparent on multiparent.codeval = ' + tbl + '.' + lovfield.name;
...
exports.putModelForm = function (ent, model, fields, keys, sql_extfields, sql_extvalues, encryptedfields, enc_datalockqueries, param_datalocks) {
  var sql = '';
  var enc_sql = '';
  ...
  var sql_values = _.map(fields_insert, function (field) { return '@' + field.name; }).concat(sql_extvalues).join(',');
  ...
  if (encryptedfields.length > 0) {
    if (!('sqlinsertencrypt' in model)) {
      var tbl = model.table;
      enc_sql = 'update ' + tbl + ' set ' + _.map(encryptedfields, function (field) { var rslt = field.name + '=@' + (field.name); if ('hash' in field) rslt += ',' + field.hash + '=@' + field.hash; return rslt; }).join(',');
...
exports.postModelForm = function (ent, model, fields, keylist, sql_extfields, sql_extvalues, param_datalocks, datalockqueries) {
  ...
  if (!('sqlupdate' in model)) {
    var tbl = model.table;
    sql = 'update ' + tbl + ' set ' + _.map(_.filter(fields,function(field){ return (field.sqlupdate!==''); }), function (field) { if (field && field.sqlupdate) return field.name + '=' + parseSQL(ent, field.sqlupdate); return field.name + '=@' + (field.name); }).join(',');
...
exports.getLOV = function (ent, fname, lov, datalockqueries, param_datalocks) {
  ...
  else if ('UCOD' in lov) { sql = 'select ' + ent.map.codeval + ',' + ent.map.codetxt + ' from UCOD_' + lov['UCOD'] + ' where (CODETDT is null or CODETDT>getdate()) order by ' + ent.map.codeseq + ',' + ent.map.codetxt; }
  else if ('UCOD2' in lov) { sql = 'select ' + ent.map.codeval + '1 as ' + ent.map.codeparent + ',' + ent.map.codeval + '2 as ' + ent.map.codeval + ',' + ent.map.codetxt + ' from UCOD2_' + lov['UCOD2'] + ' where (CODETDT is null or CODETDT>getdate()) order by ' + ent.map.codeseq + ',' + ent.map.codetxt; }
  else if ('GCOD' in lov) { sql = 'select ' + ent.map.codeval + ',' + ent.map.codetxt + ' from GCOD_' + lov['GCOD'] + ' where (CODETDT is null or CODETDT>getdate()) order by ' + ent.map.codeseq + ',' + ent.map.codetxt; }
  else if ('GCOD2' in lov) { sql = 'select ' + ent.map.codeval + '1 as ' + ent.map.codeparent + ',' + ent.map.codeval + '2 as ' + ent.map.codeval + ',' + ent.map.codetxt + ' from GCOD2_' + lov['GCOD2'] + ' where (CODETDT is null or CODETDT>getdate()) order by ' + ent.map.codeseq + ',' + ent.map.codetxt; }
