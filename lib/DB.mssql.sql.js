...
DBsql.prototype.getLOVFieldTxt = function (ent, model, field) {
  ...
  if(lov.values){
    if('parent' in lov) rslt = "(select " + ent.map.codetxt + " from (" + _this.arrayToTable(DB.util.ParseLOVValues(ent, lov.values)) + ") "+field.name+"_values where "+field.name+"_values."+ent.map.codeval+"1=(" + parentsql + ") and "+field.name+"_values."+ent.map.codeval+"2=(" + valsql + "))";
    else rslt = "(select " + ent.map.codetxt + " from (" + _this.arrayToTable(DB.util.ParseLOVValues(ent, lov.values)) + ") "+field.name+"_values where "+field.name+"_values."+ent.map.codeval+"=(" + valsql + "))";
  }
