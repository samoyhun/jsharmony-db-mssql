...
DBsql.prototype.getModelRecordset = function (ent, model, sql_filterfields, allfields, sortfields, searchfields, datalockqueries,
                                      rowstart, rowcount) {
  var _this = this;
  var sql = '';
  var rowcount_sql = '';
  ...
  sql_suffix = ' from ' + _this.getTable(ent, model) + ' where ';
  
  //Generate SQL Suffix (where condition)
  if (('sqlwhere' in model) && model.sqlwhere) sql_suffix += _this.ParseSQL(model.sqlwhere);
  else sql_suffix += '1=1';
  _.each(sql_filterfields, function (field) {
    if ('sqlwhere' in field) sql_suffix += ' and ' + _this.ParseSQL(field.sqlwhere);
  ...
  if (!('sqlselect' in model)) {
    ...
    for (var i = 0; i < allfields.length; i++) {
      ...
      if ('lov' in field) sql += ',' + _this.getLOVFieldTxt(ent, model, field);
    }
    if(this.db.dbconfig.version == "mssql2008"){
      var sql2 = '';
      _.each(allfields, function (val) { sql2 += (sql2 ? ',' : '') + val.name; });
      sql = 'select ' + sql2 + ' from (' + sql + (allfields.length ? ',' : '') + 'ROW_NUMBER() over (order by %%%SORT%%%) as _ROW_NUMBER' + sql_suffix + ') as _X2008TBL where _X2008TBL._ROW_NUMBER BETWEEN (%%%ROWSTART%%%+1) and (%%%ROWSTART%%% + %%%ROWCOUNT%%% ) order by %%%SORT%%%';
    }
    else {
      sql += sql_suffix + ' order by %%%SORT%%% offset %%%ROWSTART%%% rows fetch next %%%ROWCOUNT%%% rows only';
    }
  }
  else sql = _this.ParseSQL(model.sqlselect);
  if (!('sqlrowcount' in model)) {
    rowcount_sql = 'select count(*) as cnt' + sql_suffix;
  }
  else rowcount_sql = _this.ParseSQL(model.sqlrowcount);
...
DBsql.prototype.getModelForm = function (ent, model, selecttype, allfields, sql_filterfields, datalockqueries, sortfields) {
  var _this = this;
  var sql = '';
  ...
  if (!('sqlselect' in model)) {
    ...
    sql += ' from ' + tbl + ' where ';
    if (('sqlwhere' in model) && model.sqlwhere) sql += _this.ParseSQL(model.sqlwhere);
    else sql += '1=1';
    sql += ' %%%DATALOCKS%%%';
...
DBsql.prototype.getModelMultisel = function (ent, model, lovfield, allfields, sql_filterfields, datalockqueries, lov_datalockqueries, param_datalocks) {
  var _this = this;
  var sql = '';
  ...
  if (!('sqlselect' in model)) {
    ...
    //Add Keys to where
    if (sql_filterfields.length) _.each(sql_filterfields, function (field) { sql += ' and ' + field.name + '=' + XtoDB(ent, field, '@' + field.name); });
    else sql += ' and 0=1';
    sql += ') ' + tbl_alias;
...
DBsql.prototype.putModelForm = function (ent, model, fields, keys, sql_extfields, sql_extvalues, encryptedfields, hashfields, enc_datalockqueries, param_datalocks) {
  ...
  if (!('sqlinsert' in model)) {
    ...
    sql = _this.ParseSQL(model.sqlinsert);
  ...
  if (encryptedfields.length > 0) {
    ...
    else enc_sql = _this.ParseSQL(model.sqlinsertencrypt);
...
DBsql.prototype.postModelForm = function (ent, model, fields, keys, sql_extfields, sql_extvalues, hashfields, param_datalocks, datalockqueries) {
  ...
  else sql = _this.ParseSQL(model.sqlupdate);
...
DBsql.prototype.postModelMultisel = function (ent, model, lovfield, lovvals, filterfields, param_datalocks, datalockqueries, lov_datalockqueries) {
  var _this = this;
  var sql = '';
...
DBsql.prototype.deleteModelForm = function (ent, model, keys, datalockqueries) {
  ...
  else sql = _this.ParseSQL(model.sqldelete);
...
DBsql.prototype.Download = function (ent, model, fields, keys, datalockqueries) {
  ...
  else sql = _this.ParseSQL(model.sqldownloadselect);
...
DBsql.prototype.getSearchTerm = function (ent, field, pname, search_value, comparison) {
  var _this = this;
  var sqlsearch = '';
  var fsql = field.name;
  if (field.sqlselect) fsql = field.sqlselect;
  if (field.sql_search){
    fsql = ent.parseFieldExpression(field, _this.ParseSQL(field.sql_search), { SQL: fsql });
  }
  ...
  switch (ftype) {
    ...
    case 'char':
      ...
      else if ((comparison == 'soundslike') && (field.sql_search_sound)) { sqlsearch = _this.ParseSQL(field.sql_search_sound).replace('%%%FIELD%%%', pname_param); }
...
DBsql.prototype.getLOV = function (ent, fname, lov, datalockqueries, param_datalocks, options) {
  ...
  if(options.truncate_lov){ sql = 'select * from (' + sql + ') __lov where ' + ent.map.codeval + '=@__' + ent.map.codeval; }
...
DBsql.prototype.getBreadcrumbTasks = function (ent, model, datalockqueries, bcrumb_sql_fields) {
  var _this = this;
  var sql = _this.ParseSQL(model.breadcrumbs.sql);
...
DBsql.prototype.getTable = function(ent, model){
  ...
  if(model.table=='jsharmony:models'){
    ...
    for(var _modelid in ent.Models){
      ...
      rslt += "'" + _this.escape(_model.component) + "',";
