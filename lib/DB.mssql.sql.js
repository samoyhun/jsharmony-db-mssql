...
DBsql.prototype.getModelRecordset = function (jsh, model, sql_searchfields, allfields, sortfields, searchfields, datalockqueries,
                                      rowstart, rowcount) {
  var _this = this;
  var sql = '';
  var rowcount_sql = '';
...
DBsql.prototype.getModelMultisel = function (jsh, model, lovfield, allfields, sql_foreignkeyfields, datalockqueries, lov_datalockqueries, param_datalocks) {
  ...
  else if ('code' in lov) { lovsql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ',' + jsh.map.code_seq + ' from ' + jsh.map.code + '_' + lov['code'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>getdate())'; }
  else if ('code_sys' in lov) { lovsql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ',' + jsh.map.code_seq + ' from ' + jsh.map.code_sys + '_' + lov['code_sys'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>getdate())'; }
  else if ('code_app' in lov) { lovsql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ',' + jsh.map.code_seq + ' from ' + jsh.map.code_app + '_' + lov['code_app'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>getdate())'; }
...
DBsql.prototype.postModelForm = function (jsh, model, fields, keys, sql_extfields, sql_extvalues, hashfields, param_datalocks, datalockqueries) {
  ...
  var sqlwhere = '1=1';
  if(jsh && jsh.Config && jsh.Config.system_settings && jsh.Config.system_settings.deprecated && jsh.Config.system_settings.deprecated.disable_sqlwhere_on_form_update_delete){ }
...
DBsql.prototype.postModelMultisel = function (jsh, model, lovfield, lovvals, foreignkeyfields, param_datalocks, datalockqueries, lov_datalockqueries) {
  ...
  if (lovvals.length > 0) {
    ...
    for (var i = 0; i < lovvals.length; i++) { if (i > 0) sql += ','; sql += XtoDB(jsh, lovfield, '@multisel' + i); }
    sql += ')';
  }
  sql += ' %%%DATALOCKS%%%; ';
  if (lovvals.length > 0) {
...
DBsql.prototype.getLOV = function (jsh, fname, lov, datalockqueries, param_datalocks, options) {
  ...
  else if ('code' in lov) { sql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ jsh.map.code + '_' + lov['code'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>getdate()) order by ' + jsh.map.code_seq + ',' + jsh.map.code_txt; }
  else if ('code2' in lov) { sql = 'select ' + jsh.map.code_val + '1 as ' + jsh.map.code_parent + ',' + jsh.map.code_val + '2 as ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ jsh.map.code2 + '_' + lov['code2'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>getdate()) order by ' + jsh.map.code_seq + ',' + jsh.map.code_txt; }
  else if ('code_sys' in lov) { sql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ jsh.map.code_sys + '_' + lov['code_sys'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>getdate()) order by ' + jsh.map.code_seq + ',' + jsh.map.code_txt; }
  else if ('code2_sys' in lov) { sql = 'select ' + jsh.map.code_val + '1 as ' + jsh.map.code_parent + ',' + jsh.map.code_val + '2 as ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ jsh.map.code2_sys + '_' + lov['code2_sys'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>getdate()) order by ' + jsh.map.code_seq + ',' + jsh.map.code_txt; }
  else if ('code_app' in lov) { sql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ jsh.map.code_app + '_' + lov['code_app'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>getdate()) order by ' + jsh.map.code_seq + ',' + jsh.map.code_txt; }
  else if ('code2_app' in lov) { sql = 'select ' + jsh.map.code_val + '1 as ' + jsh.map.code_parent + ',' + jsh.map.code_val + '2 as ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ jsh.map.code2_app + '_' + lov['code2_app'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>getdate()) order by ' + jsh.map.code_seq + ',' + jsh.map.code_txt; }
...
function XfromDB(jsh, field, fieldsql){
  ...
  if(field.type && field.sql_from_db){
    var rslt = jsh.parseFieldExpression(field, field.sql_from_db, { SQL: (fieldsql?'('+fieldsql+')':'') });
  }
  //Simplify
  if(rslt == field.name) {}
...
function XtoDB(jsh, field, fieldsql){
  ...
  if(field.type && field.sql_to_db){
    var rslt = jsh.parseFieldExpression(field, field.sql_to_db, { SQL: (fieldsql?'('+fieldsql+')':'') });
  }
  return rslt;
}
...
function XSearchtoDB(jsh, field, fieldsql){
  ...
  if(field.type && field.sqlsearch_to_db){
    var rslt = jsh.parseFieldExpression(field, field.sqlsearch_to_db, { SQL: (fieldsql?'('+fieldsql+')':'') });
  }
  return rslt;
}
