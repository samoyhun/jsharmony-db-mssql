...
DBsql.prototype.getModelRecordset = function (ent, model, sql_searchfields, allfields, sortfields, searchfields, datalockqueries,
  ...
  for (var i = 0; i < allfields.length; i++) {
    ...
    if (field.lov) sql += ',' + _this.getLOVFieldTxt(ent, model, field) + ' as __' + ent.map.code_txt + '__' + field.name;
  }
...
DBsql.prototype.getModelMultisel = function (ent, model, lovfield, allfields, sql_foreignkeyfields, datalockqueries, lov_datalockqueries, param_datalocks) {
  ...
  sql += ' ,isnull(' + ent.map.code_val + ',' + lovfield.name + ') ' + ent.map.code_val + ',isnull(isnull(' + ent.map.code_txt + ',' + ent.map.code_val + '),' + lovfield.name + ') ' + ent.map.code_txt;
  ...
  sql += ') ' + tbl_alias;
  sql += ' full outer join (%%%LOVSQL%%%) multiparent on multiparent.' + ent.map.code_val + ' = ' + tbl_alias + '.' + lovfield.name;
  sql += ' order by ' + ent.map.code_seq + ',' + ent.map.code_txt;
  if('sqlselect' in model) sql = _this.ParseSQL(model.sqlselect).replace('%%%SQL%%%', sql);
  ...
  else if ('code' in lov) { lovsql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ',' + ent.map.code_seq + ' from ' + ent.map.code + '_' + lov['code'] + ' where (' + ent.map.code_end_date + ' is null or ' + ent.map.code_end_date + '>getdate())'; }
  else if ('code_sys' in lov) { lovsql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ',' + ent.map.code_seq + ' from ' + ent.map.code_sys + '_' + lov['code_sys'] + ' where (' + ent.map.code_end_date + ' is null or ' + ent.map.code_end_date + '>getdate())'; }
  else if ('code_app' in lov) { lovsql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ',' + ent.map.code_seq + ' from ' + ent.map.code_app + '_' + lov['code_app'] + ' where (' + ent.map.code_end_date + ' is null or ' + ent.map.code_end_date + '>getdate())'; }
...
DBsql.prototype.putModelForm = function (ent, model, fields, keys, sql_extfields, sql_extvalues, encryptedfields, hashfields, enc_datalockqueries, param_datalocks) {
  ...
  var sql_values = _.map(fields_insert, function (field) { if(field.sqlinsert) return field.sqlinsert; return XtoDB(ent, field, '@' + field.name); }).concat(sql_extvalues).join(',');
  var tbl = _this.getTable(ent, model);
  sql = 'insert into ' + tbl + '(' + sql_fields + ') ';
  sql += ' values(' + sql_values + ');';
  //Add Keys to where
  //Assume that the first key not part of the insert fields is the scope_identity
  var keys_in_insert = [];
  var identity_key = undefined;
  for(var i=0;i<keys.length;i++){
    var found_key = false;
    for(var j=0;j<fields_insert.length;j++){
      if(fields_insert[j]==keys[i]){ found_key = true; break; }
    }
    if(!found_key) identity_key = keys[i];
    else keys_in_insert.push(keys[i]);
  }
  //If two keys are not part of the insert fields, generate an error
  if ((keys.length == 1) && identity_key) sql += 'select SCOPE_IDENTITY() as ' + keys[0].name + ';';
  else if (keys.length >= 1){
    sql += 'select ';
    sql += _.map(keys, function (field) { return field.name + ' as ' + field.name; }).join(',');
    sql += ' from '+tbl+' where ';
    if(identity_key) sql += identity_key.name+'=SCOPE_IDENTITY()';
    else sql += _.map(keys_in_insert, function (field) { return field.name + ' = ' + XtoDB(ent, field, '@' + field.name); }).join(' and ');
    sql += ';';
  }
...
DBsql.prototype.postModelMultisel = function (ent, model, lovfield, lovvals, foreignkeyfields, param_datalocks, datalockqueries, lov_datalockqueries) {
  ...
  if (lovvals.length > 0) {
    ...
    sql += ent.map.code_val + ' from (%%%LOVSQL%%%) multiparent where ' + ent.map.code_val + ' in ('
    for (var i = 0; i < lovvals.length; i++) { if (i > 0) sql += ','; sql += XtoDB(ent, lovfield, '@multisel' + i); }
    sql += ') and ' + ent.map.code_val + ' not in (select ' + lovfield.name + ' from ' + tbl + ' where 1=1 ';
    _.each(foreignkeyfields, function (field) { sql += ' and ' + field.name + '=' + XtoDB(ent, field, '@' + field.name); });
    sql += ' %%%DATALOCKS%%%);'
  }
  sql += 'select @@rowcount as xrowcount;';
...
DBsql.prototype.getLOV = function (ent, fname, lov, datalockqueries, param_datalocks, options) {
  ...
  else if ('code' in lov) { sql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code + '_' + lov['code'] + ' where (' + ent.map.code_end_date + ' is null or ' + ent.map.code_end_date + '>getdate()) order by ' + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('code2' in lov) { sql = 'select ' + ent.map.code_val + '1 as ' + ent.map.code_parent + ',' + ent.map.code_val + '2 as ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code2 + '_' + lov['code2'] + ' where (' + ent.map.code_end_date + ' is null or ' + ent.map.code_end_date + '>getdate()) order by ' + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('code_sys' in lov) { sql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code_sys + '_' + lov['code_sys'] + ' where (' + ent.map.code_end_date + ' is null or ' + ent.map.code_end_date + '>getdate()) order by ' + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('code2_sys' in lov) { sql = 'select ' + ent.map.code_val + '1 as ' + ent.map.code_parent + ',' + ent.map.code_val + '2 as ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code2_sys + '_' + lov['code2_sys'] + ' where (' + ent.map.code_end_date + ' is null or ' + ent.map.code_end_date + '>getdate()) order by ' + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('code_app' in lov) { sql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code_app + '_' + lov['code_app'] + ' where (' + ent.map.code_end_date + ' is null or ' + ent.map.code_end_date + '>getdate()) order by ' + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('code2_app' in lov) { sql = 'select ' + ent.map.code_val + '1 as ' + ent.map.code_parent + ',' + ent.map.code_val + '2 as ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code2_app + '_' + lov['code2_app'] + ' where (' + ent.map.code_end_date + ' is null or ' + ent.map.code_end_date + '>getdate()) order by ' + ent.map.code_seq + ',' + ent.map.code_txt; }
  else sql = 'select 1 as ' + ent.map.code_val + ',1 as ' + ent.map.code_txt + ' where 1=0';
...
DBsql.prototype.getLOVFieldTxt = function (ent, model, field) {
  ...
  if(lov.values){
    if('parent' in lov) rslt = "(select " + ent.map.code_txt + " from (" + _this.arrayToTable(DB.util.ParseLOVValues(ent, lov.values)) + ") "+field.name+"_values where "+field.name+"_values."+ent.map.code_val+"1=(" + parentsql + ") and "+field.name+"_values."+ent.map.code_val+"2=(" + valsql + "))";
    else rslt = "(select " + ent.map.code_txt + " from (" + _this.arrayToTable(DB.util.ParseLOVValues(ent, lov.values)) + ") "+field.name+"_values where "+field.name+"_values."+ent.map.code_val+"=(" + valsql + "))";
  }
  else if ('sqlselect' in lov) { rslt = _this.ParseSQL(lov['sqlselect']); }
  else if ('code' in lov) { rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code + '_' + lov['code'] + ' where ' + ent.map.code_val + '=(' + valsql + ')'; }
  else if ('code2' in lov) {
    if (!parentsql) throw new Error('Parent field not found in LOV.');
    rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code2 + '_' + lov['code2'] + ' where ' + ent.map.code_val + '1=(' + parentsql + ') and ' + ent.map.code_val + '2=(' + valsql + ')';
  }
  else if ('code_sys' in lov) { rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code_sys + '_' + lov['code_sys'] + ' where ' + ent.map.code_val + '=(' + valsql + ')'; }
  else if ('code2_sys' in lov) {
    if (!parentsql) throw new Error('Parent field not found in LOV.');
    rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code2_sys + '_' + lov['code2_sys'] + ' where ' + ent.map.code_val + '1=(' + parentsql + ') and ' + ent.map.code_val + '2=(' + valsql + ')';
  }
  else if ('code_app' in lov) { rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code_app + '_' + lov['code_app'] + ' where ' + ent.map.code_val + '=(' + valsql + ')'; }
  else if ('code2_app' in lov) {
    if (!parentsql) throw new Error('Parent field not found in LOV.');
    rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ ent.map.code2_app + '_' + lov['code2_app'] + ' where ' + ent.map.code_val + '1=(' + parentsql + ') and ' + ent.map.code_val + '2=(' + valsql + ')';
  }
