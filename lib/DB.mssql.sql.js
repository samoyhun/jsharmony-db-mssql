...
exports.getModelRecordset = function (ent, model, sql_filterfields, allfields, sortfields, searchfields, datalockqueries,
  ...
  if (!('sqlselect' in model)) {
    ...
    if(global.dbconfig && (global.dbconfig.version == "mssql2008")){
      var sql2 = '';
      _.each(_allfields, function (val) { sql2 += (sql2 ? ',' : '') + val.name; });
      sql = 'select ' + sql2 + ' from (' + sql + (_allfields.length ? ',' : '') + 'ROW_NUMBER() over (order by %%%SORT%%%) as _ROW_NUMBER' + sql_suffix + ') as _X2008TBL where _X2008TBL._ROW_NUMBER BETWEEN (%%%ROWSTART%%%+1) and (%%%ROWSTART%%% + %%%ROWCOUNT%%% ) order by %%%SORT%%%';
    }
    else {
...
exports.postModelForm = function (ent, model, fields, keylist, sql_extfields, sql_extvalues, param_datalocks, datalockqueries) {
  ...
  if (!('sqlupdate' in model)) {
    var tbl = model.table;
    sql = 'update ' + tbl + ' set ' + _.map(fields, function (field) { if (field && field.sqlupdate) return field.name + '=' + parseSQL(ent, field.sqlupdate); return field.name + '=@' + (field.name); }).join(',');
