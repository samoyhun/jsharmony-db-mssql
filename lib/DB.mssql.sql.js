...
DBsql.prototype.putModelForm = function (ent, model, fields, keys, sql_extfields, sql_extvalues, encryptedfields, hashfields, enc_datalockqueries, param_datalocks) {
  ...
  if (keys.length >= 1){
    ...
    if(model._dbdef && (model._dbdef.table_type=='view') && (model._dbdef.instead_of_insert) && (keys.length == 1)){ /* Insert key should be provided by view */ }
      ...
      else sqlgetinsertkeys += _.map(keys_in_insert, function (field) { return field.name + ' = ' + XtoDB(ent, field, '@' + field.name); }).join(' and ');
      sqlgetinsertkeys += ';';
    }
...
DBsql.prototype.postModelForm = function (ent, model, fields, keys, sql_extfields, sql_extvalues, hashfields, param_datalocks, datalockqueries) {
  ...
  var sqlwhere = '1=1';
  if(ent && ent.Config && ent.Config.system_settings && ent.Config.system_settings.deprecated && ent.Config.system_settings.deprecated.disable_sqlwhere_on_form_update_delete){ }
...
DBsql.prototype.postModelMultisel = function (ent, model, lovfield, lovvals, foreignkeyfields, param_datalocks, datalockqueries, lov_datalockqueries) {
  ...
  if (lovvals.length > 0) {
    ...
    for (var i = 0; i < lovvals.length; i++) { if (i > 0) sql += ','; sql += XtoDB(ent, lovfield, '@multisel' + i); }
    sql += ') and ' + ent.map.code_val + ' not in (select ' + lovfield.name + ' from ' + tbl + ' where (%%%SQLWHERE%%%) ';
    _.each(foreignkeyfields, function (field) { sql += ' and ' + field.name + '=' + XtoDB(ent, field, '@' + field.name); });
    sql += ' %%%DATALOCKS%%%);'
  }
  sql += 'select @@rowcount as xrowcount;';
...
DBsql.prototype.getLOVFieldTxt = function (ent, model, field) {
  ...
  if(lov.values){
    ...
    else if('parent' in lov) rslt = "(select " + ent.map.code_txt + " from (" + _this.arrayToTable(DB.util.ParseLOVValues(ent, lov.values)) + ") "+field.name+"_values where "+field.name+"_values."+ent.map.code_val+"1=(" + parentsql + ") and "+field.name+"_values."+ent.map.code_val+"2=(" + valsql + "))";
