...
DBdriver.prototype.Init = function (onInitialized, numRetries, onFail) {
  ...
  if (!this.con) {
    _this.con = new mssql.Connection(global.dbconfig, function (err) {
      if (err){
        DB.log('MSSQL Pool Error: ' + err.toString());
    ...
    if (numRetries > 100) { 
      DB.log('Timeout waiting for MSSQL Pool');
      if(onFail) onFail(new Error('Timeout waiting for MSSQL Pool'));
      _this.Close();
      return; 
    }
    DB.log('Retry: '+numRetries);
    setTimeout(function () { _this.Init(onInitialized, numRetries + 1, onFail); }, 100);
  }
...
DBdriver.prototype.Close = function(onClosed){
  var _this = this;
  if(_this.con && _this.IsConnected){
    _this.IsConnected = false;
    _this.con.close(function(){
      _this.con = null;
...
DBdriver.prototype.ExecSession = function (dbtrans, constring, session) {
  ...
  if (dbtrans) {
    ...
    if (constring && (constring != global.dbconfig)) {
      ...
      _this.Init(
        function () { 
          session(null, _this.con, global.dbconfig._presql || '', function () { }); 
        }, 
        0,
        function(err){ return ExecError(err, session, "DB Connect Error: "); }
      );
    }
  }
}
...
DBdriver.prototype.Exec = function (dbtrans, context, return_type, sql, ptypes, params, callback, constring) {
  ...
  _this.ExecSession(dbtrans, constring, function (err, con, presql, conComplete) {
    ...
    r.on('error', function (err) {
      ...
      if (global.debug_params && global.debug_params.db_error_sql_state && no_errors){
        no_errors = false;
        DB.log('SQL Error: ' + sql + ' ' + JSON.stringify(ptypes) + ' ' + JSON.stringify(params));
      }
    });
...
DBdriver.prototype.ExecTransTasks = function (execTasks, callback, constring) {
  var _this = this;
