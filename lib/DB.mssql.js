...
DBdriver.prototype.getDBType = function (dbtype,desc) {
  ...
  if (dbtype.name == 'VarChar') {
    if ((dbtype.length == types.MAX)||(dbtype.length == -1)) return mssql.NVarChar(mssql.MAX);
    return mssql.NVarChar(dbtype.length);
  }
  else if (dbtype.name == 'Char') {
    if ((dbtype.length == types.MAX)||(dbtype.length == -1)) return mssql.Char(mssql.MAX);
    return mssql.Char(dbtype.length);
  }
  else if (dbtype.name == 'VarBinary') {
    if ((dbtype.length == types.MAX)||(dbtype.length == -1)) return mssql.VarBinary(mssql.MAX);
