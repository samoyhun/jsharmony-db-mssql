...
DBdriver.prototype.Init = function (onInitialized, numRetries) {
  ...
  if (!this.con) {
    _this.con = new mssql.Connection(global.dbconfig, function (err) {
      if (err) log('MSSQL Pool Error: ' + err.toString());
      else {
    ...
    if (numRetries > 100) { log('Timeout waiting for MSSQL Pool'); return; }
...
DBdriver.prototype.ExecSession = function (dbtrans, constring, session) {
  ...
  if (dbtrans) {
    session(dbtrans, '', function () { });
  }
  else {
    if (constring && (constring != global.dbconfig)) {
      initConstring(constring);
      var con = new mssql.Connection(constring, function (err) {
        if (err) { return ExecError(err, callback, "DB Connect Error: "); }
        session(con, constring._presql || '', function () { con.close(); });
      });
    }
    else {
      initConstring(global.dbconfig);
      _this.Init(function () { session(_this.con, global.dbconfig._presql || '', function () { }); });
    }
  }
}

function ExecError(err, callback, errprefix) {
  if (global.debug_params && global.debug_params.db_error_sql_state) log((errprefix || '') + err.toString());
...
DBdriver.prototype.Exec = function (dbtrans, context, return_type, sql, ptypes, params, callback, constring) {
  ...
  _this.ExecSession(dbtrans, constring, function (con, presql, conComplete) {
    //Non-streaming
    /*
	  ...
	  r.query('select * from GCOD_C_BCTGR where CODEVAL=@CODEVAL',function(err,rs){
		  if(typeof err != 'undefined') log(err);
		  else console.dir(rs);
	  });
	  */
	  //r.execute
    ...
    r.on('error', function (err) {
      ...
      if (global.debug_params && global.debug_params.db_error_sql_state) log('SQL Error: ' + sql + ' ' + JSON.stringify(ptypes) + ' ' + JSON.stringify(params));
    });
...
DBdriver.prototype.ExecTransTasks = function (dbtasks, callback, constring) {
  var _this = this;
  _this.ExecSession(null, constring, function (con, presql, conComplete) {
