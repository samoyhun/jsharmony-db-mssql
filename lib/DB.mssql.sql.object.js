...
DBObjectSQL.prototype.drop = function(jsh, module, obj){
  ...
  if(obj.type=='table'){
    ...
    if(obj.code_type && (obj.code_type=='app')) code_type = 'app';
    sql += "if (object_id('"+this.sql.escape(obj.name)+"', 'U') is not null) drop table "+(obj.name)+";\nGO\n";
    sql += "delete from "+jsHarmonyFactorySchema+jsh.map['code_'+code_type]+" where code_name='"+this.sql.escape(codename)+"' and code_schema=case '{schema}' when '' then null else '{schema}' end;\n";
  }
  else if(obj.type=='code2'){
    var jsHarmonyFactorySchema = this.getjsHarmonyFactorySchema(jsh);
    if(jsHarmonyFactorySchema) jsHarmonyFactorySchema += '.';
    ...
    if(obj.code_type && (obj.code_type=='app')) code_type = 'app';
    sql += "if (object_id('"+this.sql.escape(obj.name)+"', 'U') is not null) drop table "+(obj.name)+";\nGO\n";
    sql += "delete from "+jsHarmonyFactorySchema+jsh.map['code2_'+code_type]+" where code_name='"+this.sql.escape(codename)+"' and code_schema=case '{schema}' when '' then null else '{schema}' end;\n";
  }
  if(sql) sql += 'GO\n'
  return sql;
}
