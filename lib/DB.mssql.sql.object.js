...
DBObjectSQL.prototype.init = function(jsh, module, obj){
  ...
  if(obj.type=='table'){
    ...
    sql += "exec sys.sp_addextendedproperty @name=N'MS_Description', @value=N'"+this.sql.escape(caption[2])+"' , @level0type=N'SCHEMA',@level0name=N'"+this.sql.escape(tableschema?tableschema:'dbo')+"', @level1type=N'TABLE',@level1name=N'"+this.sql.escape(tablename)+"';\nGO\n";
  }
    ...
    sql += "insert into "+jsHarmonyFactorySchema+jsh.map['code_'+code_type]+" (code_name, code_desc, code_schema, code_type) VALUES ('"+this.sql.escape(codename)+"', '"+this.sql.escape(caption[2])+"', case '{schema}' when '' then null else '{schema}' end, '"+code_type+"');\nGO\n";
    sql += "exec "+jsHarmonyFactorySchema+"create_code_"+code_type+" "+(codeschema?"'"+this.sql.escape(codeschema)+"'":'null')+",'"+this.sql.escape(codename)+"','"+this.sql.escape(caption[2])+"';\nGO\n";
  }
    ...
    var code_type = 'sys';
    if(obj.code_type && (obj.code_type=='app')) code_type = 'app';
    sql += "insert into "+jsHarmonyFactorySchema+jsh.map['code2_'+code_type]+" (code_name, code_desc, code_schema, code_type) VALUES ('"+this.sql.escape(codename)+"', '"+this.sql.escape(caption[2])+"', case '{schema}' when '' then null else '{schema}' end, '"+code_type+"');\nGO\n";
    sql += "exec "+jsHarmonyFactorySchema+"create_code2_"+code_type+" "+(codeschema?"'"+this.sql.escape(codeschema)+"'":'null')+",'"+this.sql.escape(codename)+"','"+this.sql.escape(caption[2])+"';\nGO\n";
  }
...
DBObjectSQL.prototype.getKeyJoin = function(obj, tbl1, tbl2, options){
  options = _.extend({ no_errors: false, cursor: false }, options);
  var primary_keys = [];
  var joinexp = [];
...
DBObjectSQL.prototype.resolveTrigger = function(obj, type){
  ...
  if(sql || rowsql){
    ...
    if(sql) sql = this.db.ParseSQLFuncs(sql, objFuncs);
    if(rowsql) rowsql = this.db.ParseSQLFuncs(rowsql, objFuncs);
    ...
    presql += ' from deleted full outer join inserted on '+_this.getKeyJoin(obj,'inserted','deleted', { no_errors: true }).join(' and ');
    presql += ';\n';
...
DBObjectSQL.prototype.restructureInit = function(jsh, module, obj){
  ...
  _.each(trigger_types, function(trigger_type){
    ...
    if(obj.type=='table'){
      ...
      if(trigger_type=='insert'){
        sql += 'create trigger '+obj.name+'_on_insert on '+obj.name+' instead of insert as\n'
        sql += 'begin\n' + triggers.insert + '\nend\nGO\n';
      }
      if(trigger_type=='update'){
        sql += 'create trigger '+obj.name+'_on_update on '+obj.name+' instead of update as\n'
        sql += 'begin\n' + triggers.update + '\nend\nGO\n';
      }
      if(trigger_type=='delete'){
        sql += 'create trigger '+obj.name+'_on_delete on '+obj.name+' instead of delete as\n'
        sql += 'begin\n' + triggers.delete + '\nend\nGO\n';
      }
    }
  });
...
DBObjectSQL.prototype.drop = function(jsh, module, obj){
  ...
  if(obj.type=='table'){
    ...
    if(obj.code_type && (obj.code_type=='app')) code_type = 'app';
    sql += "if (object_id('"+this.sql.escape(obj.name)+"', 'U') is not null) drop table "+(obj.name)+";\nGO\n";
    sql += "delete from "+jsHarmonyFactorySchema+jsh.map['code_'+code_type]+" where code_name='"+this.sql.escape(codename)+"' and isnull(code_schema,'') = '{schema}';\n";
  }
  else if(obj.type=='code2'){
    var jsHarmonyFactorySchema = this.getjsHarmonyFactorySchema(jsh);
    if(jsHarmonyFactorySchema) jsHarmonyFactorySchema += '.';
    ...
    if(obj.code_type && (obj.code_type=='app')) code_type = 'app';
    sql += "if (object_id('"+this.sql.escape(obj.name)+"', 'U') is not null) drop table "+(obj.name)+";\nGO\n";
    sql += "delete from "+jsHarmonyFactorySchema+jsh.map['code2_'+code_type]+" where code_name='"+this.sql.escape(codename)+"' and isnull(code_schema,'') = '{schema}';\n";
  }
  if(sql) sql += 'GO\n'
  return sql;
}
