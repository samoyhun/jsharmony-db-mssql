...
DBObjectSQL.prototype.init = function(jsh, module, obj){
  ...
  if(obj.type=='table'){
    ...
    if(obj.columns) for(var i=0; i<obj.columns.length;i++){
      ...
      if(column.foreignkey){
        ...
        for(var tbl in column.foreignkey){
          ...
          var foreignkey = ' constraint fk_'+column.name+' foreign key ('+column.name+') references '+tbl+'('+foreignkey_col.column+')';
    ...
    for(var tblname in obj.tables){
      ...
      _.each(tbl.columns, function(col){
        ...
        if(col.sqlselect){
          cols.push('(' + col.sqlselect + ') as ' + col.name);
        }
        else {
  ...
  if(sql) sql = this.db.ParseSQLFuncs(sql, triggerFuncs);
  return sql;
}
